...
from tensorflow.keras.layers import Conv2D, MaxPooling2D, AveragePooling2D, Flatten, Dense,\
    Input, BatchNormalization, Concatenate, Dropout
# Helper libraries
import numpy as np
...
#f_image_train = tf.repeat(f_label_train, 3, axis=3)
f_image_test = tf.repeat(f_image_test, 3, axis=3, name=None)
#f_image_test = tf.repeat(f_label_test, 3, axis=3, name=None)
...
def Inception(x, filters):
    ...
    '''http://www.tensorflow.org/api_docs/python/tf/keras/layers/Conv2D'''
    ''' 1x1 convolution'''
    path1 = Conv2D(filters=filters[0], kernel_size=(1,1), strides=1, padding = 'same', activation = 'relu')(x)
    
    ''' 1x1 convolution -> 3x3 convolution '''
    path2 = Conv2D(filters=filters[1][0], kernel_size=(1,1), strides=1, padding = 'same', activation = 'relu')(x)
    path2 = Conv2D(filters=filters[1][1], kernel_size=(3,3), strides=1, padding = 'same', activation = 'relu')(x)

    ''' 1x1 convolution -> 5x5 convolution '''
    path3 = Conv2D(filters=filters[2][0], kernel_size=(1,1), strides=1, padding = 'same', activation = 'relu')(x)
    path3 = Conv2D(filters=filters[2][1], kernel_size=(5,5), strides=1, padding = 'same', activation = 'relu')(x)
    
    ''' 3x3 convolution -> 1x1 convolution '''
    path4 = MaxPooling2D(pool_size=(3,3), strides=1, padding = 'same')(x)
    path4 = Conv2D(filters=filters[3], kernel_size=(1,1), strides=1, padding = 'same', activation = 'relu')(x)
...
model_fin.save('GoogleNet_mnist.h5')import tensorflow as tf
from tensorflow.keras import datasets, layers, models, Model
from tensorflow.keras.layers import Conv2D, MaxPooling2D, AveragePooling2D, Flatten, Dense,\
    Input, BatchNormalization, Concatenate, Dropout
# Helper libraries
