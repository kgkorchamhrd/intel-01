...
# class_name = ['T_shirt/top', 'Trouser', 'Pullover', 'Dress', 'Coat',
#               'Sandal', 'Shirt', 'Sneaker', 'Bag', 'Ankle boot']

    path2=Conv2D(filters=filters[1][0], kernel_size=(1,1), strides=1, padding = 'same', activation='relu')(x)
    path2=Conv2D(filters=filters[1][1], kernel_size=(3,3), strides=1, padding = 'same', activation='relu')(path2)

    path3 = Conv2D(filters=filters[2][0], kernel_size=(3,3), strides=1, padding='same', activation='relu')(x)
...
# model = tf.keras.layers.Rescaleing(1./255)(model)
...
model = Inception(model, [128, (128, 192), (16, 32), 64])
model = MaxPooling2D(pool_size=(3,3), strides=2, padding='same')(model)
