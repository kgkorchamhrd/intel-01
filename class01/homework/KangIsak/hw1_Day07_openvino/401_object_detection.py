...
if not converted_model_path.exists():
    ...
    del ov_model
print('convert_vinos_model_done\n')
# Load the Model
    
import ipywidgets as widgets

core = ov.Core()
print('vino_core_set_done\n\n ')
...
# First, list all available classes and create colors for them. Then, 
# in the post-process stage, transform boxes with normalized coordinates [0, 1] 
# into boxes with pixel coordinates [0, image_size_in_px]. Afterward, 
# use non-maximum suppression to reject overlapping detections and 
# those below the probability threshold (0.5). 
# Finally, draw boxes and labels inside them.
...
# If you have multiple webcams, each one will be assigned a consecutive number starting at 0. 
# Set flip=True when using a front-facing camera. Some web browsers, especially Mozilla Firefox, 
# may cause flickering. If you experience flickering, set use_popup=True.

# NOTE: To use this notebook with a webcam, you need to run the notebook on a computer with a webcam. 
# If you run the notebook on a server (for example, Binder), the webcam will not work. 
# Popup mode may not work if you run this notebook on a remote computer (for example, Binder).
